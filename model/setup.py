...
class Setup:
    ...
    def __setup_group(self, db):
        ...
        for g in WS.get_comparison_conf(WS.GROUPS, self.app):
            group = Group(
                name=g[WS.GROUP_NAME],
                display_name=g[WS.GROUP_DISPLAY_NAME]
            )
    ...
    def __setup_item(self, db, group, g):
        ...
        for i in g[WS.GROUP_ITEMS]:
            ...
            item = db.session.query(Item).\
                where(Item.name == i[WS.ITEM_NAME],
                      Item.display_name == i[WS.ITEM_DISPLAY_NAME],
                      Item.image_path == i[WS.ITEM_IMAGE_NAME]).first()
            # Insert the item in the database if it doesn't exists
            if item is None:
                item = Item(name=i[WS.ITEM_NAME],
    ...
    def __setup_item_group(self, db, item, group):
        ...
        item_group = db.session.query(ItemGroup).\
            where(
                ...
                ItemGroup.group_id == group_id).first()
        ...
        if item_group is None:
            ...
            self.app.logger.info("Reusing Item {} relationship with group {}."
    ...
    def __setup_user(self, db):
        ...
        for f in user_conf:
            ...
            required = bool(f[WS.USER_FIELD_REQUIRED])
            ...
            if type == WS.USER_FIELD_TYPE_DROPDOWN or type == WS.USER_FIELD_TYPE_RADIO:
                max_size = max([len(x) for x in f[WS.USER_FIELD_SELECT_OPTION]])
                col = Column(name, String(max_size), nullable=required)
            if type == WS.USER_FIELD_TYPE_INT:
        ...
        render_ethics = WS.should_render(WS.BEHAVIOR_RENDER_ETHICS_AGREEMENT_PAGE, self.app)
        if render_ethics:
    ...
    def __setup_website_control_history(self, db):
        ...
        A new setup will be necessary and the database information will be reset.
