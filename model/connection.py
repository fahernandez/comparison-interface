...
def persist(conn, obj):
    ...
    same than commiting the transaction.

    Args:
        conn (db): SQLAlchemy connection
        obj (db): Database object to be persited in the database.

    Returns:
        db: The object after refresing the persisted values as the primary keys
    """
